...
class MockLoadTest:
    ...
    async def run_load_test(self, num_requests: int = 100, concurrency: int = 10):
        ...
        logger.info(
            f"Starting load test: {num_requests} requests, {concurrency} concurrent"
        )
    ...
    def analyze_results(self, total_requests: int, total_time: float) -> Dict[str, Any]:
        ...
        successful_requests = len(
            [code for code in self.status_codes if 200 <= code < 300]
        )
...
async def main():
    ...
    for scenario in test_scenarios:
        ...
        await test.run_load_test(
            num_requests=scenario["requests"], concurrency=scenario["concurrency"]
        )
