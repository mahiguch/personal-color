...
class GeminiPromptTester:
    ...
    async def test_personal_color_analysis(self, image_path: str, expected_type: str) -> Dict:
        """パーソナルカラー診断テスト"""
        print(f"\n🧪 テスト実行: {image_path}")
    ...
    def evaluate_result(self, result: Dict, expected_type: str) -> Dict:
        ...
        if "diagnosis_result" in result:
            if result["diagnosis_result"] == expected.get("diagnosis_result"):
                ...
                evaluation["issues"].append(f"診断結果が期待値と異なる: {result['diagnosis_result']} != {expected.get('diagnosis_result')}")
        else:
        ...
        # 3. フォーマット準拠評価 (20点)
        required_fields = ["diagnosis_result", "confidence", "explanation", "recommended_colors", "avoid_colors", "tips"]
        present_fields = sum(1 for field in required_fields if field in result)
        evaluation["format_compliance"] = int((present_fields / len(required_fields)) * 20)
    ...
    async def run_comprehensive_test(self):
        ...
        for image_type, image_path in SAMPLE_IMAGES.items():
            ...
            test_result = await self.test_personal_color_analysis(image_path, expected_type)
    ...
    def generate_test_report(self):
        ...
        successful_tests = sum(1 for result in self.test_results if "error" not in result)
    ...
    def generate_improvement_suggestions(self):
        ...
        if low_scores:
            print("⚠️ 改善が必要な項目:")
            avg_accuracy = sum(r["evaluation"]["accuracy_score"] for r in low_scores) / len(low_scores)
            avg_explanation = sum(r["evaluation"]["explanation_quality"] for r in low_scores) / len(low_scores)
