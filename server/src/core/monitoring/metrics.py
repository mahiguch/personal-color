    ...
    async def get_metrics(self) -> Dict[str, Any]:
        ...
        async with self._lock:
            ...
            for endpoint, durations in self.request_duration.items():
                if durations:
                    metrics["average_response_times"][endpoint] = sum(durations) / len(durations)
...
class PerformanceMonitor:
    ...
    async def get_performance_summary(self) -> Dict[str, Any]:
        ...
        if self.response_times:
            summary["avg_response_time"] = sum(self.response_times) / len(self.response_times)
            sorted_times = sorted(self.response_times)
            p95_index = int(len(sorted_times) * 0.95)
            summary["p95_response_time"] = sorted_times[p95_index] if p95_index < len(sorted_times) else 0.0
