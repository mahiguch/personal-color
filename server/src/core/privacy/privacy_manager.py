...
class PrivacyFilter:
    ...
    def filter_image_data(data: str) -> str:
        ...
        if len(data) > 100 and ('data:image' in data or data.replace(' ', '').isalnum()):
...
class PrivacyManager:
    ...
    def create_privacy_compliant_response(self, diagnosis_result: Dict[str, Any]) -> Dict[str, Any]:
        """プライバシー準拠のレスポンス作成"""
        # レスポンスから機密情報を除去
        filtered_result = {}
        
        for key, value in diagnosis_result.items():
            if key in ['personal_color_type', 'confidence', 'explanation', 'recommended_colors', 'tips']:
                # 診断結果は含める
                filtered_result[key] = value
            elif key == 'metadata':
                # メタデータは個人情報をフィルタ
                if isinstance(value, dict):
                    filtered_result[key] = self.privacy_filter.filter_personal_info(value)
            # その他の機密データは除外
        
        return filtered_result
    
    def log_api_access(self, request_id: str, client_ip: str, endpoint: str, user_agent: Optional[str] = None):
        """APIアクセスログ（プライバシー準拠）"""
        # IPアドレスをハッシュ化
    ...
    def validate_data_minimization(self, request_data: Dict[str, Any]) -> List[str]:
        ...
        if 'metadata' in request_data:
            ...
            if isinstance(metadata, dict):
                unnecessary_fields = set(metadata.keys()) - {'timestamp', 'app_version', 'device_type'}
                if unnecessary_fields:
                    warnings.append(f"Unnecessary metadata fields detected: {unnecessary_fields}")
