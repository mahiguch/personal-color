...
class AppCheckMiddleware:
    ...
    def __init__(self, project_id: str, skip_verification: bool = False):
        """
        Args:
    ...
    def _verify_app_check_token(self, request: Request) -> None:
        ...
        try:
            ...
            if decoded_token.get("aud") != [self.project_id]:
                logger.warning(
                    f"Invalid project ID in token: {decoded_token.get('aud')}"
                )
                raise HTTPException(
                    status_code=status.HTTP_401_UNAUTHORIZED,
            ...
            logger.debug(
                f"App Check token verified for app: {decoded_token.get('app_id')}"
            )
