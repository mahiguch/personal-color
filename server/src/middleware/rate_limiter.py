...
class AdaptiveRateLimiter:
    ...
    async def is_allowed(
        self, identifier: str, is_error: bool = False
    ...
    def _adjust_limits(self):
        ...
        if self.total_requests > 0:
            ...
            if error_rate > 0.1:  # エラー率10%超過で制限強化
                ...
                logger.warning(
                    f"Rate limit tightened to {self.current_limit} due to high error rate: {error_rate:.2%}"
                )
            elif error_rate < 0.02:  # エラー率2%未満で制限緩和
                self.current_limit = min(self.base_limit, int(self.current_limit * 1.1))
                logger.info(
                    f"Rate limit relaxed to {self.current_limit} due to low error rate: {error_rate:.2%}"
                )
...
class RateLimitMiddleware(BaseHTTPMiddleware):
    ...
    def __init__(
        ...
        self.default_limiter = SlidingWindowRateLimiter(
            max_requests=default_requests_per_minute, window_seconds=60
        )

        # 診断エンドポイント用（シンプルな制限）
        self.diagnosis_limiter = SlidingWindowRateLimiter(
            max_requests=diagnosis_requests_per_minute, window_seconds=60
        )
    ...
    async def dispatch(self, request: Request, call_next: Callable) -> Response:
        ...
        if not await self._check_burst_limit(client_id):
            return self._create_rate_limit_response(
                "Too many requests in short time", retry_after=10
            )
    ...
    def _add_rate_limit_headers(self, response: Response, endpoint_type: str):
        """レート制限ヘッダーを追加"""
        if endpoint_type == "diagnosis":
            response.headers["X-RateLimit-Limit-Diagnosis"] = str(
                self.diagnosis_limiter.max_requests
            )
        else:
            response.headers["X-RateLimit-Limit-General"] = str(
                self.default_limiter.max_requests
            )
