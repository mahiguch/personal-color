    ...
    def _initialize_vertex_ai(self):
        ...
        try:
            ...
            generation_config = GenerationConfig(
                max_output_tokens=2048, temperature=0.3, top_p=0.8, top_k=40
            )
...
必ずJSON形式で回答し、正確な診断を行ってください。""",
    ...
    async def analyze_personal_color(
        self, image: ProcessedImage, metadata: Optional[Dict[str, Any]] = None
    ) -> "PersonalColorResult":
        """
        画像からパーソナルカラーを診断
        ...
        try:
            ...
            # 3. Gemini APIで分析実行
            content_parts = [analysis_prompt, gemini_image]
            ...
            logger.info(
                f"パーソナルカラー診断成功: タイプ={result.personal_color_type}, "
        ...
        for attempt in range(max_retries):
            try:
                ...
                response = await loop.run_in_executor(
                    None, self._model.generate_content, content_parts
                )
                    ...
                    if hasattr(response, "prompt_feedback"):
                        ...
                        if hasattr(feedback, "block_reason"):
                            raise GeminiServiceError(
                                f"プロンプトがブロックされました: {feedback.block_reason}"
                            )
    ...
    def get_metrics(self) -> Dict[str, Any]:
        ...
        success_rate = (
            (self._metrics["successful_requests"] / total * 100) if total > 0 else 0.0
        )
