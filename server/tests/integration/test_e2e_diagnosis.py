...
class TestE2EDiagnosis:
    ...
    def test_complete_diagnosis_flow_success(self, client, valid_test_image):
        ...
        with patch("src.services.gemini.gemini_service.vertexai"), patch(
            ...
            request_data = {
                ...
                "metadata": {"app_version": "1.0.0", "device_type": "iPhone"},
            }
    ...
    def test_invalid_request_handling(self, client):
        ...
        response = client.post(
            "/api/v1/diagnose",
            json={"image_base64": "invalid_base64_data!@#$", "metadata": {}},
        )
        assert response.status_code == 422

        # Test missing image data
        response = client.post(
            "/api/v1/diagnose", json={"image_base64": "", "metadata": {}}
        )
        assert response.status_code == 422
